...
string Board::AsString(bool useNumbers, bool showUnfixed) const
{
	...
	for (int i = 0; i < numCells; i++)
	{
		...
		if (cellContents.size() > maxLen )
			maxLen = static_cast<unsigned int>(cellContents.size());
...
{
	// Directly set the cell value, even if it's already fixed
	// This bypasses the check in SetCell that prevents overwriting fixed cells
	bool wasFixed = cells[i].Fixed();
	cells[i] = c;
	
	// Update bookkeeping - increment if newly fixed, decrement if was fixed but now isn't
	if (cells[i].Fixed() && !wasFixed)
		++numFixedCells;
	else if (!cells[i].Fixed() && wasFixed)
		--numFixedCells;
